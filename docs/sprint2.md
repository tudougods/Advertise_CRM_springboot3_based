...
- `V10__enforce_recharge_order_terminal_fields.sql`（板块 E review 后约束订单终态字段一致）
...
- [x] 实现订单创建、查询和合法状态迁移。
- [x] 实现仅在本地/测试环境启用的模拟支付入口。
- [x] 实现 HMAC 验签、时间窗口校验和回调审计。
- [x] 实现回调、订单更新、账户入账、流水落库的单事务处理。
- [x] 测试签名错误、金额不符、订单不存在、重复回调和并发回调。
