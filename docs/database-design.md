...
| 删除 | 用户、广告主和分类提供物理删除；日常停用优先使用 `status`，删除关联对象时外键置为 `NULL` |
...
2. 待审批或禁用用户不能登录。
3. 至少保留一个 `ACTIVE ADMIN`；不能降级、禁用或删除最后一个可用管理员，并通过行锁串行化并发变更。
4. 只能给广告主分配状态为 `ACTIVE` 的分类和负责人。
5. 分类或用户被禁用时，不自动修改已有广告主关系。
6. 企业名称和注册编号在写入前去除首尾空格；用户名和邮箱同时转为小写。
7. 用户、广告主和分类的 `DELETE` 接口执行物理删除；可恢复的停用操作使用 `PATCH` 修改状态。
